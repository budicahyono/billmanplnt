              ...
              <div class="card-body">
                ...
                <table id="data_tusbung" class="table table-bordered table-hover ">
                  ...
                  <tr >
                    ...
                    <th colspan=2>TUL TUSBUNG (DLM BULAN INI)</th>
                  ...
                  foreach ($petugas->result() as $r) {
                    ...
                    $sum_tul =  $this->M_Tusbung->get_by_petugas($r->id_petugas, $id_unit)->num_rows(); 
                    ...
                    $sum_lunas = $this->M_Tusbung->get_lunas_tgl($r->id_petugas, $id_unit, $tgl_skrg)->num_rows();
                    
                    $total_lunas = $total_lunas + $sum_lunas;
                    
                   $sum_lunas_rp = $this->M_Tusbung->get_lunas_tgl_rp($r->id_petugas, $id_unit, $tgl_skrg);
                    ...
                    $sum_blm = $sum_tul - $sum_lunas;
                    $sum_blm_rp = $sum_rp - $lunas_rp;
                    
                    $total_blm = $total_blm + $sum_blm;
                    $total_blm_rp = $total_blm_rp + $sum_blm_rp;
                  ...
                  <tr>
                    ...
                    <td><a href="javascript:void(0)" id="blm_<?=$no?>" data-name="<?=$r->nama_petugas?>" data-id="<?=$r->id_petugas?>" data-sum="<?=$sum_blm?>" data-jenis="blm" ><?=$sum_blm?></td>
                    <td><?=rp($sum_blm_rp)?></td>
                  ...
                  <?php $no = $no + 1;} foreach ($petugas_khusus->result() as $r) { 
                    ...
                    $sum_tul =  $this->M_Tusbung->get_by_petugas(null, $id_unit, $id_petugas_khusus)->num_rows(); 
                    
                    $total_tul = $total_tul + $sum_tul;
                    
                    
                    $cek_tul_rp =  $this->M_Tusbung->get_tul_petugas_rp(null, $id_unit, $id_petugas_khusus);
                     foreach ($cek_tul_rp->result() as $row) {
						$sum_rp = $row->rptag;
					  } 
                    ...
                    $sum_lunas = $this->M_Tusbung->get_lunas_tgl(null, $id_unit, $tgl_skrg, $id_petugas_khusus)->num_rows();
                    
                    $total_lunas = $total_lunas + $sum_lunas;
                    
                    $sum_lunas_rp = $this->M_Tusbung->get_lunas_tgl_rp(null, $id_unit, $tgl_skrg, $id_petugas_khusus);
                    foreach ($sum_lunas_rp->result() as $row) {
						$lunas_rp = $row->rptag;
                    } 
                  ...
                  <?php $no = $no + 1;} foreach ($non_petugas->result() as $r) { 
                    $id_petugas = $r->id_petugas;
                    $sum_tul =  $this->M_Tusbung->get_by_petugas($id_petugas, $id_unit)->num_rows(); 
                    
                    $total_tul = $total_tul + $sum_tul;
                    
                     $cek_tul_rp =  $this->M_Tusbung->get_tul_petugas_rp($id_petugas, $id_unit);
                     foreach ($cek_tul_rp->result() as $row) {
						$sum_rp = $row->rptag;
					  } 
                    ...
                    $sum_lunas = $this->M_Tusbung->get_lunas_tgl($id_petugas, $id_unit, $tgl_skrg )->num_rows();
                    
                    $total_lunas = $total_lunas + $sum_lunas;
                    
                    $sum_lunas_rp = $this->M_Tusbung->get_lunas_tgl_rp($id_petugas, $id_unit, $tgl_skrg);
                    foreach ($sum_lunas_rp->result() as $row) {
						$lunas_rp = $row->rptag;
                    } 
