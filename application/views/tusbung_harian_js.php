...
$(document).ready(function() {  
  ...
  function ajax_tusbung(id_petugas, jenis = null, limit = null, q = null) { //petugas umum
    if (limit == null) {
      isi_limit = 10;
    } else {
      isi_limit = limit;
    ...
    $.ajax({
			dataType: 'json',
			url: "<?php echo base_url(); ?>tusbung_harian/petugas/" + id_petugas + "?tgl=<?=$tgl_skrg?>&jenis="+jenis+"&limit="+ isi_limit + q_url,
  ...
  function detail(id){
      ...
      $.ajax({
            dataType: 'json',
			url: "<?php echo base_url(); ?>tusbung_harian/detail/" + id,
			success: function(data) {
  ...
  $("#search_<?=$no?>").keyup(function(){
    ...
    if (search == "") {
      ...
      $.ajax({
            dataType: 'json',
			url: "<?php echo base_url(); ?>tusbung_harian/search/" + id_petugas + "?tgl=<?=$tgl_skrg?>&jenis="+jenis+"&q="+ search,
			success: function(data) {
                $('#auto_<?=$no?>').empty();
                var no = 1;
                data.data_rows.forEach(function(data_rows) {
...
$(document).ready(function() {  
  ...
  //function panggil data pelanggan  
  function ajax_tusbung(id_petugas_khusus, jenis = null, limit = null, q = null) {
    if (limit == null) {
      isi_limit = 10;
    } else {
      isi_limit = limit;
    ...
    $.ajax({
			dataType: 'json',
			url: "<?php echo base_url(); ?>tusbung_harian/petugas?tgl=<?=$tgl_skrg?>&id_k="+id_petugas_khusus+"&jenis="+jenis+"&limit="+ isi_limit + q_url,
			success: function(data) {
                $('#tabel_isi_<?=$no?>').empty();
                var no = 1;
                data.data_rows.forEach(function(data_rows) {
                    ...
                    if (data.total == null) {
                        row += '<td><a href="javascript:void(0)" id="detail_'+id_petugas_khusus+'_'+no+'"   data-id="'+data_rows.id_pelanggan+'"  >'+data_rows.id_pelanggan+'</a></td>';
                    } else {
                      row += '<td><a href="javascript:void(0)" id="detail_total_'+no+'"   data-id="'+data_rows.id_pelanggan+'"  >'+data_rows.id_pelanggan+'</a></td>';
                    } 
                    ...
                    if (data.total == null) {
                        $("#detail_"+id_petugas_khusus+"_"+no).click(function(){
                            var id_pelanggan = $(this).data("id");
                            detail(id_pelanggan);
  ...
  $("#tul_<?=$no?>, #lunas_<?=$no?>, #blm_<?=$no?>").click(function(){
    var nama_petugas = $(this).data("name");
    var id_petugas_khusus = $(this).data("id");
    var sum = $(this).data("sum");
    var jenis = $(this).data("jenis");
    if (jenis == "tul") {
        var title_jenis = "Semua Pelanggan";
    ...
    $("#id_petugas_<?=$no?>").val(id_petugas_khusus);
    $("#sum_<?=$no?>").val(10);
    $("#jenis_<?=$no?>").val(jenis);
    var nama = nama_petugas.toLowerCase();
    ...
    $("#direction_<?=$no?>").fadeIn();
    ajax_tusbung(id_petugas_khusus, jenis);
  ...
  //jquery ketika user pilih limit data
  $("#limit_<?=$no?>").change(function(){
    var id_petugas_khusus = $("#id_petugas_<?=$no?>").val();
    var limit =  $("#limit_<?=$no?>").val();
    var jenis =  $("#jenis_<?=$no?>").val();
    $("#sum_<?=$no?>").val(limit);
    $("#modal_app_<?=$no?>").modal();
    ajax_tusbung(id_petugas_khusus, jenis, limit)
    $("#isi_modal2_<?=$no?>, #isi_modal_<?=$no?>").show();
    $("#isi_detail_<?=$no?>").hide(); 
  ...
  $("#load_<?=$no?>").click(function(){
    ...
    $("#sum_<?=$no?>").val(sum);
    $("#modal_app_<?=$no?>").modal();
    ajax_tusbung(id_petugas_khusus, jenis, sum)
    $("#isi_modal2_<?=$no?>, #isi_modal_<?=$no?>").show();
    $("#isi_detail_<?=$no?>").hide(); 
  ...
  $("#search_<?=$no?>").keyup(function(){
    ...
    if (search == "") {
      ...
      $.ajax({
            dataType: 'json',
			url: "<?php echo base_url(); ?>tusbung_harian/search?tgl=<?=$tgl_skrg?>&id_k="+id_petugas_khusus+"&jenis="+jenis+"&q="+ search,
			success: function(data) {
                $('#auto_<?=$no?>').empty();
                var no = 1;
                data.data_rows.forEach(function(data_rows) {
                    ...
                    $("#auto_li_<?=$no?>_"+no).click(function(){
                        var isi_auto =  $(this).data("id");
                        var id_petugas_khusus =  data.id_petugas_khusus;
                        $("#search_<?=$no?>").val(isi_auto);
                        $("#auto_<?=$no?>").css("display", "none");
                        ajax_tusbung(id_petugas_khusus, jenis, null, isi_auto)
                    })
                no = no + 1;
                })
                if (data.total_rows == 0) {
...
$(document).ready(function() {  
  ...
  function ajax_tusbung_total(jenis = null, limit = null, q = null) {
    ...
    if (q == null) {
     ...
     $.ajax({
			dataType: 'json',
			url: "<?php echo base_url(); ?>tusbung_harian/petugas?tgl=<?=$tgl_skrg?>&total=ya&jenis="+jenis+"&limit="+ isi_limit + q_url,
			success: function(data) {
  ...
  $("#search_total").keyup(function(){
    ...
    if (search == "") {
      ...
      $.ajax({
            dataType: 'json',
			url: "<?php echo base_url(); ?>tusbung_harian/search?tgl=<?=$tgl_skrg?>&total=ya&jenis="+jenis+"&q="+ search,
			success: function(data) {
                $('#auto_total').empty();
                var no = 1;
                data.data_rows.forEach(function(data_rows) {
